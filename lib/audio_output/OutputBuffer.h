...
class OutputBuffer
{
  ...
  public:
    OutputBuffer(int number_samples_to_buffer) : m_number_samples_to_buffer(number_samples_to_buffer)
    {
      ...
      // set reading and weiting to the beginning of the buffer
      m_read_head = 0;
      m_write_head = 0;
      m_available_samples = 0;
      // we'll start off buffering data as we have no samples yet
      m_buffering = true;
      // make sufficient space for the buffering and incoming data
    ...
    void add_samples(const uint8_t *samples, int count)
    {
      ...
      // copy the samples into the buffer wrappin around as needed
    ...
    void remove_samples(int16_t *samples, int count)
    {
      ...
      {
        samples[i] = 0;
        // if we have no samples and we aren't already buffering
